...
import '../../features/chat/data/chat_repository.dart' show MediaUploadTask;
...
/// Full-screen status composer for an admin-managed account: pick a photo or
/// video (camera/gallery), add an optional caption, and post. With no media the
/// caption becomes a text status. Statuses expire after 24h.
class AdminManagedStatusComposerScreen extends StatefulWidget {
  const AdminManagedStatusComposerScreen({
    super.key,
  ...
  @override
  State<AdminManagedStatusComposerScreen> createState() =>
      _AdminManagedStatusComposerScreenState();
}

class _AdminManagedStatusComposerScreenState
    extends State<AdminManagedStatusComposerScreen> {
  ...
  Future<void> _pickImage(ChatMediaSource source) async {
    try {
      final PickedChatImage? image =
          await ManagedChatScope.of(context).mediaPicker.pickImage(source: source);
  ...
  Future<void> _pickVideo(ChatMediaSource source) async {
    try {
      final PickedChatVideo? video =
          await ManagedChatScope.of(context).mediaPicker.pickVideo(source: source);
  ...
  Future<void> _post() async {
    ...
    try {
      ...
      final String statusId = 'ms_${DateTime.now().microsecondsSinceEpoch}';

      if (_image != null) {
        final PickedChatImage image = _image!;
        final MediaUploadTask task = await chat.uploadStatusMedia(
          statusId: statusId,
          bytes: image.bytes,
        ...
        if (video.thumbnailBytes != null) {
          final MediaUploadTask thumbTask = await chat.uploadStatusThumbnail(
            statusId: statusId,
...
class _ManagedSourceAction extends StatelessWidget {
  const _ManagedSourceAction({
    required this.icon,
    required this.label,
    required this.onTap,
  });
