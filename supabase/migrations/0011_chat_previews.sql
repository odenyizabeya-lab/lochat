-- LoText chat-list previews.
--
-- The conversation row already keeps a denormalized text summary
-- ("Photo", "Video", "Voice message", ...) for the chats list. This migration
-- adds the message type and media duration so the list can render richer,
-- WhatsApp-style previews:
--
--   * text   -> "You: last message…"
--   * image  -> "📷 Photo"
--   * video  -> "🎬 Video"
--   * voice  -> "🎙 Voice message (0:16)"
--
-- The client shows the leading icon and the "(m:ss)" suffix; the server only
-- needs to keep the last message's type and duration_ms in sync, which is done
-- atomically inside send_message (like the existing summary columns).
--
...
alter table public.conversations
  ...
  add column if not exists last_message_duration_ms bigint;

-- Recreate send_message to also keep last_message_type and the media duration
-- in sync. The summary text, sender, timestamp and unread counters behave
-- exactly as before.
